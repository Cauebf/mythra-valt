...
export default function ProductCard({ product }: ProductCardProps) {
  const { id, name, price, image, category, discount, rating = 4.5, reviewCount = 0 } = product
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50 mb-2">
          ...
          {category && (
            <div className="absolute top-2 right-2">
              <Badge variant="secondary" className="text-xs bg-white/90 text-gray-700 hover:bg-white/90">
                {category}
              </Badge>
            </div>
          )}
        ...
        <div className="space-y-1">
          ...
          <div className="space-y-0.5">
            <div className="flex items-baseline gap-2">
              {discount && <span className="text-lg text-red-600 font-light">-{discount}%</span>}
              <span className="text-lg font-normal text-gray-900">{formattedPrice}</span>
            </div>
            {originalPrice && (
